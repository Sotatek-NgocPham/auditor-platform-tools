import { InputType, Instance, Issue } from './4naly3er.types';
import { lineFromIndex } from './4naly3er.utils';
...
const analyze = (
  ...
  let result = '';
  const analyze: { issue: Issue; instances: Instance[] }[] = [];
    ...
    if (issue.regexOrAST === 'Regex') {
      for (const file of files) {
        ...
        if (!!issue.regexPreCondition) {
          const preConditionMatches: any = [
            ...file.content.matchAll(issue.regexPreCondition),
          ];
