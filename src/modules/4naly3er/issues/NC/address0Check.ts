...
import { getStorageVariable, instanceFromSRC } from '../../4naly3er.utils';
    ...
    for (const file of files) {
      if (!!file.ast) {
        for (const contract of findAll('ContractDefinition', file.ast)) {
          ...
          const storageVariables = getStorageVariable(contract);
              ...
              if (
                ...
                for (const test of findAll('BinaryOperation', func)) {
                  if (
                    ...
                    ((test.rightExpression.nodeType === 'Identifier' &&
                      test.rightExpression.name === assignName) ||
                      (test.leftExpression.nodeType === 'Identifier' &&
                        test.leftExpression.name === assignName))
                  ) {
                    check = true;
                  }
                }
