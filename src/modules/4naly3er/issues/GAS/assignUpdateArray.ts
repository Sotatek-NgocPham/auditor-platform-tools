        ...
        for (const node of findAll('Assignment', file.ast)) {
          if (
            ...
            if (node.rightHandSide.rightExpression.nodeType === 'IndexAccess') {
              if (
                (!array ||
                  (node.rightHandSide.rightExpression.baseExpression
                    .nodeType === 'Identifier' &&
                    array ===
                      node.rightHandSide.rightExpression.baseExpression
                        .name)) &&
                (!index ||
                  (node.rightHandSide.rightExpression.indexExpression
                    ?.nodeType === 'Literal' &&
                    index ===
                      node.rightHandSide.rightExpression.indexExpression
                        ?.value))
              ...
              node.rightHandSide.leftExpression.nodeType === 'IndexAccess'
            ) {
              if (
                (!array ||
                  (node.rightHandSide.leftExpression.baseExpression.nodeType ===
                    'Identifier' &&
                    array ===
                      node.rightHandSide.leftExpression.baseExpression.name)) &&
                (!index ||
                  (node.rightHandSide.leftExpression.indexExpression
                    ?.nodeType === 'Literal' &&
                    index ===
                      node.rightHandSide.leftExpression.indexExpression?.value))
              ) {
                instances.push(instanceFromSRC(file, node.src));
              }
            }
