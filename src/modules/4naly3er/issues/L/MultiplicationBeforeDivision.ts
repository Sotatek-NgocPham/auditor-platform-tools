          ...
          for (const func of findAll('FunctionDefinition', contract)) {
            for (const binaryOp of findAll('BinaryOperation', func)) {
              if (binaryOp.operator === '*') {
                if (
                  (binaryOp.leftExpression &&
                    binaryOp.leftExpression.nodeType === 'BinaryOperation' &&
                    binaryOp.leftExpression.operator === '/') ||
                  (binaryOp.rightExpression &&
                    binaryOp.rightExpression.nodeType === 'BinaryOperation' &&
                    binaryOp.rightExpression.operator === '/')
                ) {
